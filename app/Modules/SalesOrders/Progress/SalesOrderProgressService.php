...
final readonly class SalesOrderProgressService
{
    ...
    public function record(
        ...
        try {
            ...
            if ((string) $exception->getCode() === '23514') {
                throw ValidationException::withMessages([
                    'quantity_delta' => 'Sipariş progress işlemi sipariş miktarı sınırlarını aşamaz ve net progress değerini negatife düşüremez.',
                ]);
            }
    ...
    private function signedDecimal(string $value): string
    {
        ...
        if (preg_match('/^-?\d+(?:\.\d{1,6})?$/D', $value) !== 1) {
            throw ValidationException::withMessages([
                'quantity_delta' => 'Sipariş progress miktarı en fazla 6 ondalıklı geçerli bir sayı olmalıdır.',
            ]);
        }

        $unsigned = ltrim($value, '-');
        $integerPart = explode('.', $unsigned, 2)[0];
        ...
        if ($row === null || $row->valid !== true) {
            throw ValidationException::withMessages([
                'quantity_delta' => 'Sipariş progress miktarı sıfır olamaz.',
            ]);
        }
