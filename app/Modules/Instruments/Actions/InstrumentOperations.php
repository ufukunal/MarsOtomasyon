...
final readonly class InstrumentOperations
{
    ...
    public function register(
        ...
        return DB::transaction(function () use ($companyId, $accountId, $direction, $kind, $documentNo, $amount, $currencyCode, $dueDate, $issueDate, $bankName, $branchName, $drawerOrMaker, $note): Instrument {
            ...
            $instrument = Instrument::query()->create([
                ...
                'due_date' => $dueDate,
                'bank_name' => $this->nullableText($bankName, 160),
                'branch_name' => $this->nullableText($branchName, 120),
                'drawer_or_maker' => $this->nullableText($drawerOrMaker, 200),
                'note' => $this->nullableText($note, 5000),
            ]);

            $instrumentId = $this->id($instrument);
            $posting = $this->accountPoster->post(new PostAccountTransactionData(
                accountId: $accountId,
                postingDate: $issueDate ?? $dueDate,
            ...
            $this->event($instrument, 'registered', 'draft', $initialStatus, $issueDate ?? $dueDate, $account, null, $posting, null);
    ...
    public function sendToBank(int $companyId, int $instrumentId, int $treasuryAccountId, string $eventDate): Instrument
    {
        ...
        return DB::transaction(function () use ($companyId, $instrumentId, $treasuryAccountId, $eventDate): Instrument {
            ...
            $instrument->update([
                'status' => 'bank_collection',
                'current_holder_type' => 'bank',
                'current_holder_account_id' => null,
                'current_treasury_account_id' => $this->id($bank),
            ]);
    ...
    public function endorse(int $companyId, int $instrumentId, int $supplierAccountId, string $eventDate): Instrument
    {
        ...
        return DB::transaction(function () use ($companyId, $instrumentId, $supplierAccountId, $eventDate): Instrument {
            ...
            $posting = $this->accountPoster->post(new PostAccountTransactionData(
                ...
                signedAmount: (string) $instrument->amount,
            ...
            $instrument->update([
                'status' => 'endorsed',
                'current_holder_type' => 'account',
                'current_holder_account_id' => $supplierAccountId,
                'current_treasury_account_id' => null,
                'endorsed_to_account_id' => $supplierAccountId,
    ...
    public function settle(int $companyId, int $instrumentId, int $treasuryAccountId, string $eventDate): Instrument
    {
        ...
        return DB::transaction(function () use ($companyId, $instrumentId, $treasuryAccountId, $eventDate): Instrument {
            ...
            $movement = $this->treasuryPoster->post(new PostTreasuryMovementData(
                ...
                movementType: $direction === 'received' ? 'collection' : 'payment',
                accountId: (int) $instrument->account_id,
            ...
            $instrument->update([
                'status' => $targetStatus,
                'current_holder_type' => 'settled',
                'current_holder_account_id' => null,
                'current_treasury_account_id' => null,
                'settlement_treasury_account_id' => $treasuryAccountId,
                'settlement_treasury_movement_id' => $this->id($movement),
    ...
    private function reverseOpenInstrument(int $companyId, int $instrumentId, string $eventDate, string $outcome): Instrument
    {
        ...
        return DB::transaction(function () use ($companyId, $instrumentId, $eventDate, $outcome): Instrument {
            ...
            $allowed = $direction === 'received'
                ? ($outcome === 'returned' ? ['portfolio'] : ['portfolio', 'bank_collection', 'endorsed'])
                : ($outcome === 'returned' ? [] : ['issued']);
            if ($allowed === []) {
            ...
            $delivery = AccountTransaction::query()
                ->where('company_id', $companyId)
                ->findOrFail((int) $instrument->delivery_account_transaction_id);
            ...
            if ($instrument->endorsement_account_transaction_id !== null) {
                $endorsement = AccountTransaction::query()
                    ->where('company_id', $companyId)
                    ->findOrFail((int) $instrument->endorsement_account_transaction_id);
                $endorsementReversal = $this->accountPoster->post(new PostAccountTransactionData(
                    accountId: (int) $instrument->endorsed_to_account_id,
                    postingDate: $eventDate,
            ...
            $instrument->update([
                ...
                'current_holder_account_id' => $holderAccountId,
                'current_treasury_account_id' => null,
                'delivery_reversal_account_transaction_id' => $this->id($deliveryReversal),
            ...
            $this->event(
                ...
                $terminal,
                $eventDate,
                $terminal === 'returned' || $terminal === 'unpaid' ? $instrument->account()->first() : null,
                null,
                $deliveryReversal,
                null,
    ...
    private function commercialAccount(int $companyId, int $accountId, string $direction, string $currency): Account
    {
        ...
        $allowed = $direction === 'received'
            ? [AccountType::Customer, AccountType::Mixed]
            : [AccountType::Supplier, AccountType::Mixed];
    ...
    private function event(
        ...
        ?object $treasuryMovement,
        ?array $metadata = null,
    ): InstrumentEvent {
        return InstrumentEvent::query()->create([
            'company_id' => (int) $instrument->company_id,
            'instrument_id' => $this->id($instrument),
