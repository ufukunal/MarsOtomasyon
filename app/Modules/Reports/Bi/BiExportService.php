...
final class BiExportService
{
     ...
     * @return array{run_id:int,content:string,sha256:string,row_count:int,watermark:?string,format:string,schema_version:int,status:string}
     */
    public function export(
        ...
        try {
            ...
            foreach ($dataset->rows($companyId, $watermark) as $sourceRow) {
                if ((int) ($sourceRow['company_id'] ?? 0) !== $companyId) {
                    throw new DomainException('BI dataset emitted a cross-company row.');
                }

                $row = [];
                foreach ($requestedFields as $field) {
                    $value = $sourceRow[$field] ?? null;
                    $row[$field] = $definition[$field]['pii'] && ! $includePii ? '[MASKED]' : $value;
                }
                $rows[] = $row;
            }
            ...
            DB::transaction(function () use ($runId, $rows, $hash, $content, $nextWatermark, $scheduleId): void {
                DB::table('bi_export_runs')->where('id', $runId)->update([
                    ...
                    'artifact_expires_at' => now()->addDays(7),
    ...
    private function csv(array $fields, array $rows): string
    {
        ...
        foreach ($rows as $row) {
            fputcsv($stream, array_map(static function (mixed $value): string|int|float|null {
