...
@section('title', $order === null ? 'Yeni Satınalma Siparişi' : 'Satınalma Siparişi Düzenle')

@section('app-content')
<section class="workspace-hero">
    <div><p class="eyebrow">Alış / Sipariş</p><h1>{{ $order === null ? 'Yeni Satınalma Siparişi' : $order->number.' Düzenle' }}</h1><p>Toplamlar server-side M5 hesap motorunda yeniden üretilir. Depo/lokasyon hedefi opsiyoneldir; bu aşama stok hareketi üretmez.</p></div>
</section>
...
<form method="post" action="{{ $order === null ? route('purchase-orders.store') : route('purchase-orders.update', $order->getKey()) }}" class="detail-card" id="purchase-order-form" data-product-search-url="{{ route('purchase-orders.product-search') }}" data-product-price-field="purchase_price_net" data-product-price-label="Net alış fiyatı">
    @csrf
    ...
    <div class="form-grid">
        ...
        <label>Tedarikçi<select name="account_id" required><option value="">Seçin</option>@foreach($accounts as $account)<option value="{{ $account->getKey() }}" @selected((string) old('account_id', $order?->account_id) === (string) $account->getKey())>{{ $account->code }} — {{ $account->legal_name }}</option>@endforeach</select></label>
    ...
    <section class="statement-table-card">
        <table class="data-table"><thead><tr><th>Ürün</th><th>Hedef Depo</th><th>Hedef Lokasyon</th><th>Açıklama</th><th>Miktar</th><th>Fiyat</th><th>Fiyat Tipi</th><th>İskonto %</th><th>KDV Sıfırla</th><th>KDV 0 Nedeni</th></tr></thead><tbody>
        ...
        @endphp
        <tr>
            <td><div class="product-search-entry" data-product-search-entry>
                ...
                <small data-product-search-help>Ürün seçildiğinde kayıtlı net alış fiyatı önerilir; kaydetmeden önce değiştirilebilir.</small>
            </div></td>
    ...
    <div class="page-actions"><a href="{{ $order === null ? route('purchase-orders.index') : route('purchase-orders.show', $order->getKey()) }}">Vazgeç</a><button class="button-primary" type="submit">Kaydet</button></div>
</form>
@endsection
