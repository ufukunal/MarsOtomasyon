...
@section('app-content')
@php($commercialRevision = $quote->selectedRevision)
@php($displayLines = $commercialRevision?->lines ?? $quote->lines)
...
<tbody>@foreach($displayLines as $line)<tr>
...
<tfoot><tr><th colspan="6">Toplam</th><th class="amount-cell">{{ $commercialRevision?->net_total ?? $quote->net_total }}</th><th class="amount-cell">{{ $commercialRevision?->tax_total ?? $quote->tax_total }}</th><th class="amount-cell">{{ $commercialRevision?->gross_total ?? $quote->gross_total }} {{ $commercialRevision?->currency_code ?? $quote->currency_code }}</th></tr></tfoot>
</table>
</section>
...
@if(($commercialRevision?->note ?? $quote->note))<section class="detail-card"><h2>Not</h2><p>{{ $commercialRevision?->note ?? $quote->note }}</p></section>@endif
@endsection
