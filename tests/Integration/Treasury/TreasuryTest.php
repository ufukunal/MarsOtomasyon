...
it('posts customer collection and supplier payment atomically with replay-safe signs', function (): void {
    [$company, $customer, $supplier, $user, $cash, , , $cashMethod, $bankMethod] = treasury10Fixture('TR10-A');
    ...
    $this->actingAs($user)->withSession(['active_company_id' => $company->getKey()])
        ->post(route('treasury.payments.finalize', $collection->getKey()))
        ->assertRedirect();
    expect(DB::table('treasury_movements')->where('source_type', 'treasury_payment')->count())->toBe(1)
        ->and(DB::table('account_transactions')->where('source_type', 'treasury_payment')->count())->toBe(1);
...
it('blocks raw finalized payments and direct mutation of treasury authority projections', function (): void {
    [$company, $customer, , , $cash, , , $cashMethod] = treasury10Fixture('TR10-B');
...
it('settles POS gross commission net and reopens customer receivable on chargeback', function (): void {
    [$company, $customer, , $user, , $bank, $pos, , , $posMethod] = treasury10Fixture('TR10-C');
    ...
    $this->actingAs($user)->withSession(['active_company_id' => $company->getKey()])
        ->post(route('treasury.payments.finalize', $payment->getKey()))
        ->assertRedirect();
    expect($payment->refresh()->pos_status)->toBe('pending');
    ...
    expect($payment->refresh()->pos_status)->toBe('settled')
...
it('handles manual cash bank movement transfer expense and denomination cash count', function (): void {
    [$company, , , $user, $cash, $bank] = treasury10Fixture('TR10-D');
...
it('imports CSV idempotently and reconciles only an exact treasury movement', function (): void {
    [$company, , , $user, , $bank] = treasury10Fixture('TR10-E');
...
it('parses MT940 and XLSX statement fixtures without floating point amount conversion', function (): void {
    ...
    $zip = new ZipArchive;
    expect($zip->open($xlsxPath, ZipArchive::OVERWRITE))->toBeTrue();
 ...
 * }
 */
function treasury10Fixture(string $code): array
{
    $company = Company::query()->create([
        'code' => $code,
        'name' => 'Company '.$code,
    ]);
    $customer = treasury10Account($company, 'CUS', AccountType::Customer, 'Müşteri '.$code);
    $supplier = treasury10Account($company, 'SUP', AccountType::Supplier, 'Tedarikçi '.$code);
...
function treasury10Account(Company $company, string $code, AccountType $type, string $name): Account
{
