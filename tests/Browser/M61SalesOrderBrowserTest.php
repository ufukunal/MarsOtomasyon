...
it('creates, opens, and edits a manual sales order through the browser without client-side authority', function (): void {
    ...
    $page->click('Satış')
        ...
        ->select('lines[0][product_id]', (string) $product->getKey())
        ->fill('lines[0][description]', 'Browser order line')
        ->fill('lines[0][quantity]', '2')
        ->fill('lines[0][unit_price]', '100')
        ->select('lines[0][price_basis]', 'net')
        ->fill('lines[0][line_discount_rate]', '0')
        ->click('Kaydet')
        ->assertSee('SO-0001')
