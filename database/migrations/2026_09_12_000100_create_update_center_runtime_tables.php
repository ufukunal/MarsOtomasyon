...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('update_release_artifacts', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('update_run_id')->unique()->constrained('update_runs')->cascadeOnDelete();
            $table->string('package_path', 512);
            $table->string('release_path', 512);
            $table->char('package_sha256', 64);
            $table->unsignedBigInteger('package_size_bytes');
            $table->unsignedInteger('file_count');
            $table->unsignedBigInteger('extracted_size_bytes');
    ...
    {
        Schema::dropIfExists('update_release_artifacts');
