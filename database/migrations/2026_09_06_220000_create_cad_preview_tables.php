...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('cad_viewer_policies', function (Blueprint $table): void {
            ...
            $table->unsignedInteger('retention_days')->default(1);
            $table->timestampsTz();

            $table->unique(['company_id', 'provider'], 'cad_policy_company_provider_unique');
        ...
        Schema::create('cad_derivative_jobs', function (Blueprint $table): void {
            ...
            $table->string('provider', 64);
            $table->string('provider_version', 96);
            ...
            $table->unique(
                ['company_id', 'attachment_id', 'source_sha256', 'provider', 'provider_version'],
                'cad_derivative_identity_unique',
            );
            $table->index(['company_id', 'status', 'created_at'], 'cad_derivative_status_index');
            $table->foreign('company_id')->references('id')->on('companies')->cascadeOnDelete();
