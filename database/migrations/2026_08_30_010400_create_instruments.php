...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::table('permissions')->insert([
            [
                'key' => 'instruments.view',
                'name' => 'Çek / senet görüntüleme',
                'description' => 'Aktif şirketteki çek/senet portföyü, vade, holder ve lifecycle geçmişini görüntüleme yetkisi.',
                'created_at' => $now,
                'updated_at' => $now,
            ],
            [
                'key' => 'instruments.manage',
                'name' => 'Çek / senet yönetimi',
                'description' => 'Çek/senet teslim alma/verme, bankaya gönderme, ciro, tahsil/ödeme ve ters kayıt lifecycle yönetimi yetkisi.',
                'created_at' => $now,
                'updated_at' => $now,
            ],
        ]);
        ...
        Schema::create('instruments', function (Blueprint $table): void {
            ...
            $table->foreign(['company_id', 'current_holder_account_id'], 'instruments_holder_account_fk')
                ->references(['company_id', 'id'])->on('accounts')->restrictOnDelete();
            $table->foreign(['company_id', 'endorsed_to_account_id'], 'instruments_endorsed_account_fk')
                ->references(['company_id', 'id'])->on('accounts')->restrictOnDelete();
            $table->foreign(['company_id', 'current_treasury_account_id'], 'instruments_current_treasury_fk')
                ->references(['company_id', 'id'])->on('treasury_accounts')->restrictOnDelete();
            $table->foreign(['company_id', 'settlement_treasury_account_id'], 'instruments_settlement_treasury_fk')
                ->references(['company_id', 'id'])->on('treasury_accounts')->restrictOnDelete();
        ...
        DB::statement(<<<'SQL'
ALTER TABLE instruments ADD CONSTRAINT instruments_direction_status_check CHECK (
    status = 'draft'
    OR (direction = 'received' AND status IN ('portfolio','bank_collection','endorsed','collected','dishonored','returned','cancelled'))
    OR (direction = 'issued' AND status IN ('issued','settled','unpaid','cancelled'))
)
SQL);
        DB::statement(<<<'SQL'
ALTER TABLE instruments ADD CONSTRAINT instruments_registration_shape_check CHECK (
    (status = 'draft' AND delivery_account_transaction_id IS NULL AND registered_at IS NULL)
    OR (status <> 'draft' AND delivery_account_transaction_id IS NOT NULL AND registered_at IS NOT NULL)
)
SQL);
        DB::statement(<<<'SQL'
ALTER TABLE instruments ADD CONSTRAINT instruments_holder_shape_check CHECK (
    ...
    OR (status IN ('dishonored') AND current_holder_type = 'company' AND current_holder_account_id IS NULL AND current_treasury_account_id IS NULL)
...
SQL);
        ...
        Schema::create('instrument_events', function (Blueprint $table): void {
            ...
            $table->unique(['company_id', 'instrument_id', 'event_type'], 'instrument_events_once_unique');
            $table->foreign(['company_id', 'instrument_id'])
                ->references(['company_id', 'id'])->on('instruments')->restrictOnDelete();
            $table->foreign(['company_id', 'counterparty_account_id'], 'instrument_events_counterparty_fk')
                ->references(['company_id', 'id'])->on('accounts')->restrictOnDelete();
            $table->foreign(['company_id', 'treasury_account_id'], 'instrument_events_treasury_fk')
                ->references(['company_id', 'id'])->on('treasury_accounts')->restrictOnDelete();
        ...
        DB::statement("ALTER TABLE instrument_events ADD CONSTRAINT instrument_events_type_check CHECK (event_type IN ('registered','sent_to_bank','endorsed','settled','dishonored','returned','cancelled'))");
...
BEGIN
    IF OLD.status <> 'draft' AND (
        ...
        OR NEW.drawer_or_maker IS DISTINCT FROM OLD.drawer_or_maker
        OR NEW.note IS DISTINCT FROM OLD.note
    ) THEN
        RAISE EXCEPTION 'registered instrument commercial identity is immutable' USING ERRCODE = '55000';
    END IF;

    IF OLD.delivery_account_transaction_id IS NOT NULL AND NEW.delivery_account_transaction_id IS DISTINCT FROM OLD.delivery_account_transaction_id THEN
        RAISE EXCEPTION 'instrument delivery posting link is immutable' USING ERRCODE = '55000';
    END IF;
    IF OLD.endorsement_account_transaction_id IS NOT NULL AND NEW.endorsement_account_transaction_id IS DISTINCT FROM OLD.endorsement_account_transaction_id THEN
        RAISE EXCEPTION 'instrument endorsement posting link is immutable' USING ERRCODE = '55000';
    END IF;
    IF OLD.settlement_treasury_movement_id IS NOT NULL AND NEW.settlement_treasury_movement_id IS DISTINCT FROM OLD.settlement_treasury_movement_id THEN
        RAISE EXCEPTION 'instrument settlement posting link is immutable' USING ERRCODE = '55000';
    END IF;
    IF OLD.delivery_reversal_account_transaction_id IS NOT NULL AND NEW.delivery_reversal_account_transaction_id IS DISTINCT FROM OLD.delivery_reversal_account_transaction_id THEN
        RAISE EXCEPTION 'instrument delivery reversal link is immutable' USING ERRCODE = '55000';
    END IF;
    IF OLD.endorsement_reversal_account_transaction_id IS NOT NULL AND NEW.endorsement_reversal_account_transaction_id IS DISTINCT FROM OLD.endorsement_reversal_account_transaction_id THEN
        RAISE EXCEPTION 'instrument endorsement reversal link is immutable' USING ERRCODE = '55000';
    END IF;
    ...
    IF NEW.status IS DISTINCT FROM OLD.status THEN
        IF NOT (
            ...
            OR (OLD.status = 'bank_collection' AND NEW.status IN ('collected','dishonored','cancelled'))
...
CREATE TRIGGER instruments_lifecycle_guard
BEFORE UPDATE ON instruments
FOR EACH ROW EXECUTE FUNCTION mars_guard_instrument_lifecycle();
...
BEGIN
    IF NEW.delivery_account_transaction_id IS NOT NULL THEN
        ...
        IF NOT FOUND
           OR delivery.company_id IS DISTINCT FROM NEW.company_id
           OR delivery.account_id IS DISTINCT FROM NEW.account_id
           OR delivery.currency_code IS DISTINCT FROM NEW.currency_code
           OR delivery.signed_amount IS DISTINCT FROM expected_delivery
           OR delivery.source_type IS DISTINCT FROM 'instrument'
           OR delivery.source_id IS DISTINCT FROM NEW.id::text
           OR delivery.effect_type IS DISTINCT FROM 'account.instrument_delivery' THEN
    ...
    IF NEW.endorsement_account_transaction_id IS NOT NULL THEN
        ...
        IF NOT FOUND
           OR NEW.direction <> 'received'
           OR NEW.endorsed_to_account_id IS NULL
           OR endorsement.company_id IS DISTINCT FROM NEW.company_id
           OR endorsement.account_id IS DISTINCT FROM NEW.endorsed_to_account_id
           OR endorsement.currency_code IS DISTINCT FROM NEW.currency_code
           OR endorsement.signed_amount IS DISTINCT FROM NEW.amount
           OR endorsement.source_type IS DISTINCT FROM 'instrument'
           OR endorsement.source_id IS DISTINCT FROM NEW.id::text
    ...
    IF NEW.settlement_treasury_movement_id IS NOT NULL THEN
        ...
        IF NOT FOUND
           OR NEW.settlement_treasury_account_id IS NULL
           OR settlement.company_id IS DISTINCT FROM NEW.company_id
           OR settlement.treasury_account_id IS DISTINCT FROM NEW.settlement_treasury_account_id
           OR settlement.currency_code IS DISTINCT FROM NEW.currency_code
           OR settlement.signed_amount IS DISTINCT FROM expected_settlement
           OR settlement.source_type IS DISTINCT FROM 'instrument'
           OR settlement.source_id IS DISTINCT FROM NEW.id::text
...
CREATE TRIGGER instruments_ledger_link_guard
BEFORE INSERT OR UPDATE ON instruments
FOR EACH ROW EXECUTE FUNCTION mars_guard_instrument_ledger_links();

CREATE OR REPLACE FUNCTION mars_prevent_instrument_event_mutation()
RETURNS trigger AS $$
BEGIN
    RAISE EXCEPTION 'instrument_events are immutable' USING ERRCODE = '55000';
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER instrument_events_immutable_guard
BEFORE UPDATE OR DELETE ON instrument_events
FOR EACH ROW EXECUTE FUNCTION mars_prevent_instrument_event_mutation();

CREATE OR REPLACE FUNCTION mars_guard_instrument_attachment_target()
RETURNS trigger AS $$
BEGIN
    IF NEW.attachable_type = 'instrument' AND NOT EXISTS (
        SELECT 1 FROM instruments
        WHERE instruments.company_id = NEW.company_id
          AND instruments.id = NEW.attachable_id
    ) THEN
...
CREATE CONSTRAINT TRIGGER attachments_instrument_target_guard
AFTER INSERT OR UPDATE OF company_id, attachable_type, attachable_id ON attachments
DEFERRABLE INITIALLY IMMEDIATE
FOR EACH ROW EXECUTE FUNCTION mars_guard_instrument_attachment_target();
SQL);
    }
    ...
    public function down(): void
    {
        ...
        $permissionIds = DB::table('permissions')
            ->whereIn('key', ['instruments.view', 'instruments.manage'])
            ->pluck('id');
