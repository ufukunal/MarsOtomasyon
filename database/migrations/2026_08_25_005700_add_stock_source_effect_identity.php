...
if (! class_exists('AddStockSourceEffectIdentity20260825', false)) {
    final class AddStockSourceEffectIdentity20260825 extends Migration
    {
        public function up(): void
        {
            ...
            DB::statement(<<<'SQL'
                ALTER TABLE stock_movements
                ADD CONSTRAINT stock_movements_source_type_canonical_check
                CHECK (source_type ~ '^[a-z0-9]+([._-][a-z0-9]+)*$')
                SQL);
            DB::statement(<<<'SQL'
                ALTER TABLE stock_movements
                ADD CONSTRAINT stock_movements_effect_type_canonical_check
                CHECK (effect_type ~ '^[a-z0-9]+([._-][a-z0-9]+)*$')
                SQL);
            DB::statement(<<<'SQL'
                ALTER TABLE stock_movements
                ADD CONSTRAINT stock_movements_source_id_not_blank_check
                CHECK (char_length(btrim(source_id)) > 0 AND source_id = btrim(source_id))
                SQL);
...
return new AddStockSourceEffectIdentity20260825;
